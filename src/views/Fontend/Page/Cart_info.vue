...
<div class="info">
  ...
  <div class="container my-5">
    ...
    <div class="row my-5">
      ...
      <form class="col-md-7 md-spacing">
            ...
            <ValidationObserver ref="observer" v-slot="{ invalid }" tag="form">
              <ValidationProvider rules="required" name="收件人姓名" v-slot="{ errors }">
                <div class="form-group">
                  ...
                  <input class="form-control" type="text" name="name" id="username" v-model="form.user.name" placeholder="請輸入姓名" :class="{'is-invalid':errors[0]}">
              ...
              <ValidationProvider rules="required|numeric|digits:10" name="電話" v-slot="{ errors }">
                <div class="form-group">
                  ...
                  <input class="form-control" type="tel" name="tel" id="usertel" placeholder="請輸入電話" v-model="form.user.tel" :class="{'is-invalid':errors[0]}">
                  <span class="text-danger" v-if="errors[0]">{{ errors[0] }}</span>
                </div>
              </ValidationProvider>
              ...
              <ValidationProvider rules="required" name="地址" v-slot="{ errors }">
                <div class="form-group">
                  ...
                  <input class="form-control" type="text" name="name" id="useraddr" placeholder="請輸入地址" v-model="form.user.address" :class="{'is-invalid':errors[0]}">
                  <span class="text-danger" v-if="errors[0]">{{ errors[0] }}</span>
                </div>
              </ValidationProvider>
              ...
              <ValidationProvider rules="email" name="e-mail" v-slot="{ valid, errors }" ref="emailField">
                <div class="form-group">
                  ...
                  <input class="form-control" type="email" name="name" id="useremail" v-model="form.user.email" placeholder="請輸入Email取得優惠消息" :class="{'is-invalid':errors[0]}">
                  <span class="text-danger" v-if="errors[0]">{{ errors[0] }}</span>
                </div>
              </ValidationProvider>
              <div class="form-group">
                <label for="comment">留言</label>
                <textarea name="" id="comment" class="form-control" cols="30" rows="10" v-model="form.message" placeholder="備註:產品分開裝...等資訊"></textarea>
              </div>
              <div class="form-group text-right">
                <button class="btn btn-warning" :disabled="invalid" :class="{'discursor':invalid}" @click.prevent="cartCheckout">送出訂單</button>
              </div>
            </ValidationObserver>
          </div>
        </div>
